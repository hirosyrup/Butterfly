...
class MLStorage {
    func upload(uploadFileUrl: URL, fileName: String) -> Promise<String> {
        return Promise<String>(in: .background, token: nil) { (resolve, reject, _) in
            do {
                let data = try Data(contentsOf: uploadFileUrl)
    ...
    private func filesRef(fileName: String) -> StorageReference  {
        return Storage.storage().reference().child("MLFiles/\(fileName)")
    }
}
